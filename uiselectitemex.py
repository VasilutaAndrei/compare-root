	...
	if app.ENABLE_SPECIAL_STORAGE:
		def RefreshSlot(self):
			...
			for i in xrange(player.SPECIAL_SLOT_COUNT):
				...
				itemVNum = player.GetItemIndex(player.STONE_INVENTORY, slotNumber)
				if 0 == itemVNum:
					continue
				...
				itemGrade = player.GetItemGrade(player.STONE_INVENTORY, slotNumber)
				if itemGrade > 3:
					continue
			...
			for selectWndPos, inventoryPos in self.inventorySlotPosDict.items():
				itemVNum = player.GetItemIndex(player.STONE_INVENTORY, inventoryPos)
				itemCount = player.GetItemCount(player.STONE_INVENTORY, inventoryPos)
	
				if itemCount <= 1:
					itemCount = 0
	
				self.itemSlot.SetItemSlot(selectWndPos, itemVNum, itemCount)
