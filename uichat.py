...
class ChatLine(ui.EditLine):
	...
	def __SendChatPacket(self, text, type):
		if text == "(!realizare_title_1!)":
			return

		cuvinte = text.split(" ") 
		text2 = "" 
		for index in range(len(cuvinte)): 
			if "www." in cuvinte[index] or "http:" in cuvinte[index] or "https:" in cuvinte[index]: 
				tmp = cuvinte[index] 
				cuvinte[index] = "|cFF00C0FC|h|Hlink:" + tmp + "|h"+ tmp + "|h" 

			text2 += " "+cuvinte[index] 
			
		if net.IsChatInsultIn(text2[1:]):
