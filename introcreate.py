...
class CreateCharacterWindow(ui.Window):
	...
	def __CheckCreateCharacter(self, name):
		...
		if name.find("GA")!=-1:
			self.PopupMessage(localeInfo.CREATE_ERROR_GA_NAME, self.EnableWindow)
			return False

		if name.find("[")!=-1:
			self.PopupMessage(localeInfo.CREATE_ERROR_NAME, self.EnableWindow)
			return False
		...
		if name.find(localeInfo.CREATE_GM_GHILI)!=-1:
			self.PopupMessage(localeInfo.CREATE_ERROR_NAME, self.EnableWindow)
			return False
