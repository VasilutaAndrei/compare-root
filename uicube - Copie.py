...
class CubeWindow(ui.ScriptWindow):
						...
						for slotPos in xrange(self.cubeSlot.GetSlotCount()):
							...
							if not slotPos in self.cubeItemInfo:
								...
								if cubeColumn in self.cubeItemInfo:
									...
									columnItemSizeX, columnItemSizeY = item.GetItemSize(1,2,3,4,5,6)
								...
								if 0 < currentSlotLine and slotPos - self.CUBE_SLOT_COUNTX in self.cubeItemInfo:
									...
									columnItemSizeX, upperColumnItemSizeY = item.GetItemSize(1,2,3,4,5,6)
	...
	def Refresh(self):
		...
		for itemVnum, count in self.cubeResultInfos[self.firstSlotIndex:]:
			...
			# Center Align
			item.SelectItem(itemVnum)
			sizeX, sizeY = item.GetItemSize(1,2,3,4,5,6)
	...
	def __OnSelectEmptySlot(self, selectedSlotPos):
		...
		if isAttached:
			...
			chat.AppendChat(chat.CHAT_TYPE_INFO,"Pentru a adauga un item in fereastra.")
			chat.AppendChat(chat.CHAT_TYPE_INFO,"Apasa Click-Stanga pe itemul necesar din Fereastra de Rafinare")
			return
