...
if 949 == app.GetDefaultCodePage():
	...
	def DO_YOU_BUY_ITEM2(buyItemName, buyItemCount, buyItemPrice) :
		if buyItemCount > 1 :
			return DO_YOU_BUY_NEW_1_2 % ( buyItemName, buyItemCount, buyItemPrice )
		else:
			return DO_YOU_BUY_NEW_1 % ( buyItemName, buyItemPrice )
	def DO_YOU_BUY_ITEM3(buyItemName, buyItemCount, buyItemPrice) :
		if buyItemCount > 1 :
	...
	if app.ENABLE_BATTLE_FIELD:
		...
		def NumberToSecondaryCoinString(n, coinType):
			coinTypeDict = {
				shop.SHOP_COIN_TYPE_SECONDARY_COIN 	: MONETARY_UNIT_JUN,
				shop.SHOP_COIN_TYPE_BATTLE_POINT 	: BATTLE_POINT,
			}
			if coinType in coinTypeDict:
				if n <= 0 :
					return "0 %s" % (coinTypeDict[coinType])
			
				return "%s %s" % ('.'.join([ i-3<0 and str(n)[:i] or str(n)[i-3:i] for i in range(len(str(n))%3, len(str(n))+1, 3) if i ]), coinTypeDict[coinType])
			else:
				return "%s Unknown coin" % (str(n))
	else:
