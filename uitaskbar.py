...
class ExpandedMoneyTaskBar(ui.ScriptWindow):
	...
	def OnMouseOverIn(self, name):
		self.toolTip.ClearToolTip()
		self.toolTip.AppendTextLine(name, self.toolTip.SPECIAL_TITLE_COLOR)
...
class TaskBar(ui.ScriptWindow):
	...
	class SkillButton(ui.SlotWindow):
		...
		def Activate(self, coolTime):
			...
			if skill.IsToggleSkill(self.skillIndex):
				self.ActivateSlotOld(0)

		def Deactivate(self):
			if skill.IsToggleSkill(self.skillIndex):
				self.DeactivateSlotOld(0)
				...
				if player.SLOT_TYPE_INVENTORY == Type:
					...
					if constInfo.IS_AUTO_POTION(itemIndex):
						...
						if 0 != int(metinSocket[0]):
							...
							slot.DeactivateSlotOld(slotNumber)
	...
	def SelectMouseButtonEvent(self, dir, event):
		...
		if self.EVENT_MOVE == event:
			...
			import background
			if background.GetCurrentMapName() in lista_mapas:
				import chat
				chat.AppendChat(1,"No puedes hacer esto en este mapa.")
				return
